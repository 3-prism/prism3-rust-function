...
mod trait_tests {
    ...
    fn test_into_box_once() {
        ...
        assert_eq!(boxed.apply_once(20, 22), 42);
    }

    #[test]
    fn test_into_fn_once() {
        let add = BoxBiTransformerOnce::new(|x: i32, y: i32| x + y);
        let f = add.into_fn_once();
        assert_eq!(f(20, 22), 42);
    }

    #[test]
    fn test_blanket_impl_into_fn_once() {
    ...
    fn test_blanket_impl_into_fn_with_string() {
        ...
        let concat = |x: String, y: String| format!("{}-{}", x, y);
        let f = concat.into_fn_once();
        assert_eq!(f("hello".to_string(), "world".to_string()), "hello-world");
    }

    #[test]
    fn test_blanket_impl_to_box_once() {
    ...
    #[test]
    fn test_blanket_impl_to_fn_once() {
    ...
    fn test_blanket_impl_to_box_with_string() {
        ...
        let boxed = concat.to_box_once();
        assert_eq!(
            boxed.apply_once("hello".to_string(), "world".to_string()),
            "hello-world"
        );
        // Original closure still usable
    ...
    fn test_blanket_impl_to_fn_with_captured_value() {
        ...
        let f = add_with_base.to_fn_once();
...
mod trait_default_impl_tests {
    ...
    impl BiTransformerOnce<i32, i32, i32> for CustomAddTransformer {
        fn apply_once(self, first: i32, second: i32) -> i32 {
            first + second + self.offset
        }
    }

    #[test]
    fn test_trait_default_to_box_once() {
        // Test BiTransformerOnce trait's default to_box implementation
        let transformer = CustomAddTransformer { offset: 10 };
        let boxed = transformer.to_box_once();
        assert_eq!(boxed.apply_once(20, 22), 52); // 20 + 22 + 10
                                                  // Original transformer still usable
        assert_eq!(transformer.apply_once(5, 3), 18); // 5 + 3 + 10
    }

    #[test]
    fn test_trait_default_to_fn_once() {
        // Test BiTransformerOnce trait's default to_fn implementation
        let transformer = CustomAddTransformer { offset: 100 };
        let f = transformer.to_fn_once();
        assert_eq!(f(20, 22), 142); // 20 + 22 + 100
                                    // Original transformer still usable
        assert_eq!(transformer.apply_once(10, 5), 115); // 10 + 5 + 100
    }
    ...
    impl BiTransformerOnce<String, String, String> for CustomConcatTransformer {
        fn apply_once(self, first: String, second: String) -> String {
    ...
    fn test_trait_default_to_box_with_string() {
        ...
        // Original transformer still usable
        assert_eq!(
            transformer.apply_once("foo".to_string(), "bar".to_string()),
            "foo | bar"
        );
    }
    ...
    fn test_trait_default_to_fn_with_string() {
        ...
        // Original transformer still usable
        assert_eq!(
            transformer.apply_once("a".to_string(), "b".to_string()),
            "a->b"
        );
    }
    ...
    impl BiTransformerOnce<f64, f64, f64> for CustomMultiplyTransformer {
        fn apply_once(self, first: f64, second: f64) -> f64 {
    ...
    fn test_trait_default_to_box_with_float() {
        ...
        let boxed = transformer.to_box_once();
        let result = boxed.apply_once(3.0, 5.0);
        assert!((result - 20.0).abs() < 1e-10); // (3.0 + 5.0) * 2.5 = 20.0
                                                // Original transformer still usable
        let result2 = transformer.apply_once(2.0, 4.0);
    ...
    fn test_trait_default_to_fn_with_float() {
        ...
        assert!((result - 30.0).abs() < 1e-10); // (4.0 + 6.0) * 3.0 = 30.0
                                                // Original transformer still usable
        let result2 = transformer.apply_once(1.0, 2.0);
...
mod box_bi_transformer_once_tests {
    ...
    fn test_new_with_string() {
        ...
        assert_eq!(
            concat.apply_once("hello".to_string(), "world".to_string()),
            "hello world"
        );
    }
    ...
    fn test_constant_with_different_types() {
        ...
        assert_eq!(constant.apply_once("foo", "bar"), 42);
    }
    ...
    fn test_and_then_chain_multiple() {
        ...
        assert_eq!(composed.apply_once(3, 5), "Result: 16");
    }
    ...
    fn test_and_then_type_conversion() {
        ...
        assert!((composed.apply_once(6, 7) - 21.0).abs() < 1e-10);
    }
}
...
mod conditional_tests {
    ...
    fn test_when_with_or_else_condition_false() {
        ...
        let conditional = add
            .when(|x: &i32, y: &i32| *x > 0 && *y > 0)
            .or_else(multiply);
        assert_eq!(conditional.apply_once(-5, 3), -15); // multiply
    }
    ...
    fn test_when_with_complex_predicate() {
        ...
        assert_eq!(
            conditional.apply_once("hello".to_string(), "hi".to_string()),
            "hello-hi"
        );
    }
    ...
    fn test_when_with_complex_predicate_false() {
        ...
        assert_eq!(
            conditional.apply_once("hi".to_string(), "hello".to_string()),
            "hello-hi"
        );
    }
    ...
    fn test_when_both_inputs_zero() {
        ...
        let conditional = add
            .when(|x: &i32, y: &i32| *x != 0 || *y != 0)
            .or_else(constant);
        assert_eq!(conditional.apply_once(0, 0), 0); // constant
    }
    ...
    fn test_when_one_input_zero() {
        ...
        let conditional = add
            .when(|x: &i32, y: &i32| *x != 0 || *y != 0)
            .or_else(constant);
        assert_eq!(conditional.apply_once(5, 0), 5); // add
    }
}
...
mod type_tests {
    ...
    fn test_with_integers() {
        let add = BoxBiTransformerOnce::new(|x: i32, y: i32| x + y);
        assert_eq!(add.apply_once(10, 20), 30);
    }

    #[test]
    fn test_with_floats() {
        let multiply = BoxBiTransformerOnce::new(|x: f64, y: f64| x * y);
        assert!((multiply.apply_once(3.5, 2.0) - 7.0).abs() < 1e-10);
    }
    ...
    fn test_with_mixed_types() {
        ...
        assert_eq!(
            format_pair.apply_once(42, "hello".to_string()),
    ...
    fn test_with_vectors() {
        ...
        assert_eq!(merge.apply_once(vec![1, 2], vec![3, 4]), vec![1, 2, 3, 4]);
    }
    ...
    fn test_with_options() {
        ...
        assert_eq!(combine.apply_once(Some(5), Some(3)), Some(8));
    }
    ...
    fn test_with_tuples() {
        ...
        let result = swap.apply_once((42, "hello".to_string()), ("world".to_string(), 99));
        assert_eq!(
            result,
...
mod edge_case_tests {
    ...
    fn test_with_empty_strings() {
        let concat = BoxBiTransformerOnce::new(|x: String, y: String| format!("{}{}", x, y));
        assert_eq!(
            concat.apply_once(String::new(), String::new()),
            String::new()
        );
    }
    ...
    fn test_with_zero_values() {
        let add = BoxBiTransformerOnce::new(|x: i32, y: i32| x + y);
        assert_eq!(add.apply_once(0, 0), 0);
    }

    #[test]
    fn test_with_negative_values() {
        let add = BoxBiTransformerOnce::new(|x: i32, y: i32| x + y);
        assert_eq!(add.apply_once(-5, -3), -8);
    }

    #[test]
    fn test_with_large_values() {
        let add = BoxBiTransformerOnce::new(|x: i64, y: i64| x + y);
        assert_eq!(add.apply_once(1_000_000_000, 2_000_000_000), 3_000_000_000);
    }

    #[test]
    fn test_constant_ignores_inputs() {
        let constant = BoxBiTransformerOnce::constant(42);
        assert_eq!(constant.apply_once(999, 888), 42);
    }
...
mod complex_transformation_tests {
    ...
    fn test_nested_structure_transformation() {
        ...
        assert_eq!(
            merge_nested.apply_once(vec![vec![1, 2], vec![3, 4]], vec![vec![5, 6], vec![7, 8]]),
    ...
    fn test_transformation_with_calculation() {
        ...
        assert_eq!(calculate.apply_once(5, 3), (8, 15));
    }
    ...
    fn test_conditional_with_complex_logic() {
        ...
        assert_eq!(conditional.apply_once(4, 6), 20); // (4 + 6) is even, so add: 4 + 6 + 10 = 20
    }
    ...
    fn test_conditional_with_complex_logic_odd() {
        ...
        assert_eq!(conditional.apply_once(3, 4), 7); // (3 + 4) is odd, so multiply: 3 * 4 - 5 = 7
    }
}
