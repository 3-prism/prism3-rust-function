...
fn main() {
    ...
    println!("  test(&\"hello\"): {}", rc_always_true.test(&"hello".to_string()));
    println!("  test(&\"world\"): {}", rc_always_true.test(&"world".to_string()));
    println!("  name: {:?}", rc_always_true.name());

    // RcPredicate::always_false
    let rc_always_false: RcPredicate<String> = RcPredicate::always_false();
    println!("\nRcPredicate::always_false():");
    println!("  test(&\"hello\"): {}", rc_always_false.test(&"hello".to_string()));
    println!("  test(&\"world\"): {}", rc_always_false.test(&"world".to_string()));
    ...
    println!("  Clone: test(&\"test\"): {}", rc_clone.test(&"test".to_string()));
    ...
    println!("  test(&5): {} (equivalent to is_positive)", combined_and_true.test(&5));
    println!(
    ...
    println!("  test(&-3): {} (always false)", combined_and_false.test(&-3));
    ...
    println!("  test(&5): {} (equivalent to is_positive)", combined_or_false.test(&5));
    println!(
        "  test(&-3): {} (equivalent to is_positive)",
    ...
    let filtered: Vec<_> = numbers.iter().copied().filter(|value| pass_all.test(value)).collect();
    ...
    let filtered: Vec<_> = numbers.iter().copied().filter(|value| reject_all.test(value)).collect();
    println!("Default reject all elements: {:?} -> {:?}", numbers, filtered);
